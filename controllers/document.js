...
exports.update = async (req, res, next) => {
	try {
		...
		const data = await model.update(DocumentId, req.body);
		if (!_.isEmpty(data)) {
			res.status(StatusCodes.OK).send(data[0]);
		} else {
...
exports.remove = async (req, res, next) => {
	try {
		...
		//const id = req.params.id;
		const data = await model.remove(DocumentId);
		if (data) {
...
exports.getByCustomerid = async (req, res, next) => {
	try {
		...
		const totalCount = await model.getByCustomeridCount(CustomerId);
		const data = await model.getByCustomerid(offset, pageSize, CustomerId);
		if (!_.isEmpty(data)) {
			const result = {
				pageNo: pageNo,
				pageSize: pageSize,
